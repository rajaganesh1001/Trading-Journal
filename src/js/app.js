  ...
  function renderVaultToolbar() {
    ...
    Utils.id('#vaultExportCsv').addEventListener('click', () => {
      const rows = Store.listTrades().filter(vaultFilterFn()).map(t => ({
        ...
        Qty: t.qty, Charges: t.charges, Status: t.status, PnL: t.pnl ?? '', Notes: t.notes || ''
      }));
    ...
    overlay.innerHTML = `
      <div class="modal glass-panel">
        ...
        <form id="tradeForm">
          ...
          <div class="form-grid cols-3">
            <div class="field"><label>Quantity (Lots x Size)</label><input type="number" name="qty" value="${trade?.qty ?? ''}" required/></div>
          ...
          <div class="field span-2" style="margin-bottom:14px;">
            ...
            <textarea name="notes" placeholder="Setup rationale, emotional state, lessons learned...">${Utils.escapeHtml(trade?.notes || '')}</textarea>
          </div>
