...
const Store = (() => {
  ...
  function load() {
    try {
      ...
      if (raw) {
        ...
        // Migration: backfill lots/lotSize onto any trade saved before
        // that concept existed (or imported without it). normalizeTrade()
        // derives lots from the existing qty and each index's standard
        // lot size, so the trade's stored quantity — and therefore its
        // P&L — is left byte-for-byte identical; only the new lots/
        // lotSize breakdown fields are added.
        if (Array.isArray(data.trades)) {
          data.trades = data.trades.map(t => normalizeTrade(t));
        }
      } else {
        data = defaultData();
        seedDemoData();
  ...
  function normalizeTrade(input) {
    const t = { ...input };
    // Lots x Lot Size drives the actual traded quantity. Lot size defaults
    // to the standard exchange lot for the trade's index (Nifty 65 /
    // BankNifty 30 / Sensex 20) whenever it isn't explicitly provided, so
    // every trade always carries a real lots/lotSize breakdown even if it
    // came from an older code path that only ever set a plain qty.
    const hasLots = t.lots !== undefined && t.lots !== null && t.lots !== '';
    const hasLotSize = t.lotSize !== undefined && t.lotSize !== null && t.lotSize !== '';
    t.lotSize = hasLotSize ? Number(t.lotSize) || 0 : Utils.defaultLotSize(t.index);
    if (hasLots) {
      t.lots = Number(t.lots) || 0;
      t.qty = t.lots * t.lotSize;
    } else if (t.qty) {
      // Backfill path: an existing/imported trade only has a plain qty —
      // derive an equivalent lots count from it (rounded) so the lots
      // field is always meaningful going forward, without changing the
      // trade's actual P&L (qty itself is left untouched here).
      t.qty = Number(t.qty) || 0;
      t.lots = t.lotSize ? Math.round(t.qty / t.lotSize) || 1 : 1;
    } else {
      t.lots = 1;
      t.qty = t.lots * t.lotSize;
    }
